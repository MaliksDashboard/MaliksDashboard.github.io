document.addEventListener("DOMContentLoaded", () => {
  ...
  function populateMobileNavSlides() {
    ...
    mainSlides.forEach((slide, index) => {
      ...
      const heading = slide.querySelector(
        "h1, h2, .section-heading, .section-title"
      );
      const title = heading
        ? heading.textContent.substring(0, 25) +
          (heading.textContent.length > 25 ? "..." : "")
        : `Slide ${index + 1}`;
  ...
  function openMobileNav() {
    ...
    const overlay =
      document.getElementById("mobileNavOverlay") || createMobileNavOverlay();
  ...
  function scrollToSlide(index) {
    ...
    if (!isPaused && !isMobile) {
      ...
      if (duration >= 2000 && mainSlideTimeout === null) {
        // Minimum 2 seconds
        mainSlideTimeout = setTimeout(() => {
  ...
  function optimizeImages() {
    ...
    images.forEach((img) => {
      ...
      const slideIndex = Array.from(mainSlides).findIndex((slide) =>
        slide.contains(img)
      );
      if (slideIndex < 3) {
  ...
  if (!isMobile && !isPaused) {
    setTimeout(() => {
      if (!isPaused && !isMobile && mainSlides.length > 1) {
        ...
        const duration =
          parseInt(firstSlide.getAttribute("data-duration")) || 10000;
        if (duration >= 2000) {
          mainSlideTimeout = setTimeout(() => {
            if (!isPaused && !isMobile) {
              scrollToSlide(1);
